...
WHERE [CarInsuranceName].[Type] = @Parameter0";
		...
		internal DataTable GetBeenRentCarSpecTime( GetBeenRentCarSpecTime GetBeenRentCarSpecTimeParameter )
		{
			string sql = $@"select CASE " +
				...
				"where ( " +
					...
					"(not((CONVERT(datetime,@Parameter0,120) BETWEEN '" + GetBeenRentCarSpecTimeParameter.StartTime.Split( ' ' )[ 0 ] + " 9:30' AND '" + GetBeenRentCarSpecTimeParameter.StartTime.Split( ' ' )[ 0 ] + " 16:00')) and not((CONVERT(datetime,@Parameter1,120) BETWEEN '" + GetBeenRentCarSpecTimeParameter.EndTime.Split( ' ' )[ 0 ] + " 9:30' AND '" + GetBeenRentCarSpecTimeParameter.EndTime.Split( ' ' )[ 0 ] + " 16:00'))))) as CarInfo " +
				$@"Full outer join( select * from [{m_gas}].[dbo].[CarBookingRecord] where (CONVERT(datetime,@Parameter0,120) BETWEEN CONVERT(datetime,PreserveStartTime,120)  AND CONVERT(datetime,PreserveEndTime,120) and  CONVERT(datetime,@Parameter1,120) BETWEEN CONVERT(datetime,PreserveStartTime,120)  AND CONVERT(datetime,PreserveEndTime,120) and ActualEndTime IS NULL) or (CONVERT(datetime,@Parameter0,120) BETWEEN CONVERT(datetime,PreserveStartTime,120)  AND CONVERT(datetime,PreserveEndTime,120) and  CONVERT(datetime,@Parameter1,120) NOT BETWEEN CONVERT(datetime,PreserveStartTime,120)  AND CONVERT(datetime,PreserveEndTime,120) and  CONVERT(datetime,@Parameter0,120) != CONVERT(datetime,PreserveEndTime,120) and ActualEndTime IS NULL) or (CONVERT(datetime,@Parameter0,120) NOT BETWEEN CONVERT(datetime,PreserveStartTime,120)  AND CONVERT(datetime,PreserveEndTime,120) and  CONVERT(datetime,@Parameter1,120) BETWEEN CONVERT(datetime,PreserveStartTime,120)  AND CONVERT(datetime,PreserveEndTime,120) and  CONVERT(datetime,@Parameter1,120) != CONVERT(datetime,PreserveStartTime,120) and ActualEndTime IS NULL) or (CONVERT(datetime,@Parameter0,120)<CONVERT(datetime,PreserveStartTime,120) and CONVERT(datetime,@Parameter1,120)>CONVERT(datetime,PreserveEndTime,120) and ActualEndTime IS NULL)) as [CarBookingRecord] on [CarInfo].CarNumber = [CarBookingRecord].CarNumber";
		...
		internal bool InsertReserveToCarBookingRecord( InsertReserveToCarBookingRecord InsertReserveToCarBookingRecordParameter )
		{
			string sql = $@"IF @Parameter1 = 'working'
						...
						ELSE
							...
							(SELECT case when MAX([PriorityNumber]) IS NULL then '1' else MAX([PriorityNumber])+1 end as  MAXPriorityNumber FROM ( Select * FROM [{m_gas}].[dbo].[CarBookingRecord] WHERE Type='private' 
		...
		internal DataTable CheckCarInCompany( CheckCarInCompany CheckCarInCompanyParameter )
		{
			...
			List<object> SQLParameterList = new List<object>()
			{
				CheckCarInCompanyParameter.CarNumber
			};
			DataTable result = m_dbproxy.GetDataCMD( sql, SQLParameterList.ToArray() );

...
{
	public class JiraWorkLoggerdbproxy : IDBAccess
	{
		...
		public static IDBAccess GetInstance()
		{
			if( Instance == null )
			{
				...
				{ //確保單一
					if( Instance == null )
					{
						Instance = new JiraWorkLoggerdbproxy();
					}
				}
			}
			return Instance;
		...
		private JiraWorkLoggerdbproxy()
		{
			var configuration = new ConfigurationBuilder()
			.SetBasePath( $"{Directory.GetCurrentDirectory()}\\Config\\" )
			.AddJsonFile( path: "JiraWorkLoggerDBSetting.json", optional: false )
			.Build();
